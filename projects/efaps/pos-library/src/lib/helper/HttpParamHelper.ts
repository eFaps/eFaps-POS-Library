...
export class SafeHttpParams extends HttpParams {
     constructor(
    fromObject?: { [param: string]: string | number | boolean | ReadonlyArray<string | number | boolean> | null | undefined },
    codec?: HttpParameterCodec
  ) {
    // Clean the initial object before passing it to super()
    const cleanedObject = fromObject ? SafeHttpParams.removeNullOrUndefined(fromObject) : {};
